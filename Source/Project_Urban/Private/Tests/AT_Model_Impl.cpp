...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(GridPropagateNeighbours, "Project_Urban.ModelImpl.GridPropagateNeighbours",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::CriticalPriority | EAutomationTestFlags::ProductFilter)
...
bool GridPropagateNeighbours::RunTest(const FString& Parameters)
{
	TArray<UGenerationRuleset*> given_ruleset = SetupMockRulesetData();
	FGridExtentsSize given_size = FGridExtentsSize(FVector(1,1,0));
	...
	bool success = model_impl->PropagateToNeighbour(propagate_index, static_cast<int>(neighbour_adjacency));
	if(!success)
	{
	...
	int flattened_propagate_index = INCCoordinateSpaceConverter::FlattenGridArrayIndexCoordinate(propagate_index, extents_to_cell_count);
	FGridArrayIndexCoordinate neighbour_index = FGridArrayIndexCoordinate(FVector(propagate_index.vector + PUrban::ToVector(neighbour_adjacency)));
	int flattened_neighbour_index = INCCoordinateSpaceConverter::FlattenGridArrayIndexCoordinate(neighbour_index, extents_to_cell_count);
	FModelCell collapsed_cell = model_impl->D_GetGrid()[flattened_propagate_index];
	...
	TArray<ULabel*> rulesets_at_adjacency = ruleset_after_collapse->GetAdjacencyValuesFromKey(neighbour_adjacency);
	if(rulesets_at_neighbour_after_collapse.Num() != rulesets_at_adjacency.Num())
	{
		UE_LOG(LogTemp, Error, TEXT("Model propagate error! Expected number of rulesets at adjacency = {%i}, Got={%i}"),
			rulesets_at_adjacency.Num(), rulesets_at_neighbour_after_collapse.Num())
		return false;
	}
	for(int i = 0; i < rulesets_at_neighbour_after_collapse.Num(); i++)
	{
		UGenerationRuleset* current_neighbour_ruleset = rulesets_at_neighbour_after_collapse[i];
		bool contains = false;
		for(int v = 0; v < rulesets_at_adjacency.Num(); v++)
		{
			ULabel* current_desired_label = rulesets_at_adjacency[v];
		...
		if(!contains)
		{
			UE_LOG(LogTemp, Error, TEXT("Model propagate error! Ruleset found at adjacency that does not match allowed rulesets"))
			return false;
		}
	}		
